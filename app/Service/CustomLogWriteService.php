...
class CustomLogWriteService{ 
    ...
    public static function AccessLogWrite(){
        ...
        $client_ip  = CustomLogWriteService::get_client_ip();
        $client = CustomLogWriteService::getBrowser();
        if(!empty($client)) {
            $client_info = $client;
        } else {
            $client_info = [];
        ...
        $msisdn = CustomLogWriteService::MSISDNTrack();
        ...
        CustomLogWriteService::CustomLogWritter("systemlog","access_log",$message);
    ...
    public static function EventLogWrite($event_type,$event_data){
       ...
       // \DB::table('event_log')->insert($event_insert);
        ...
        CustomLogWriteService::CustomLogWritter("eventlog","event_log",$message);
    ...
    ## ApiEventLogWrite
     *********************************************/
    public static function ApiEventLogWrite($event_type,$event_data){
        ...
        CustomLogWriteService::CustomLogWritter("apieventlog","api_event_log",$message);
    ...
    public static function ErrorLogWrite($error_data){
        ...
        CustomLogWriteService::CustomLogWritter("errorlog","error_log",$message);
    ...
    ## ApiErrorLogWrite
     *********************************************/
    public static function ApiErrorLogWrite($error_data){
        ...
        CustomLogWriteService::CustomLogWritter("apierrorlog","api_error_log",$message);
...
//        $client_location  = CustomLogWriteService::geolocation($client_ip);
        $client_location = [];;
        ...
        CustomLogWriteService::CustomLogWritter("authlog","auth_log",$message);
    ...
    public static function ConvertNumberToWords($number){
        ...
        if ($number < 0) {
            return $negative . CustomLogWriteService::ConvertNumberToWords(abs($number));
        }
        ...
        switch (true) {
            ...
            case $number < 1000:
                ...
                if ($remainder) {
                    $string .= $conjunction .CustomLogWriteService::ConvertNumberToWords($remainder);
                }
                break;
            default:
                ...
                $string = CustomLogWriteService::ConvertNumberToWords($numBaseUnits) . ' ' . $dictionary[$baseUnit];
                if ($remainder) {
                    $string .= $remainder < 100 ? $conjunction : $separator;
                    $string .= CustomLogWriteService::ConvertNumberToWords($remainder);
                }
                break;
        }
...
//      $client_location  = \App\System::geolocation($client_ip);
        ...
        CustomLogWriteService::CustomLogWritter("requestlog","request_log",$message);
